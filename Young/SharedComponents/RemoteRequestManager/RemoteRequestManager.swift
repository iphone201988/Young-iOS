...
final class RemoteRequestManager {
                ...
                if let apiError = error as? APIFailureTypes {
                    ...
                    case .unAuthorizedUser:
                        DispatchQueue.main.async {
                            UserDefaults.standard.deleteData(key: .loggedUserDetails)
                            if let rootVC = AppStoryboards.main.controller(LoginVC.self) {
    ...
    fileprivate func uploadTaskParams(boundary: String, remoteRequestParams: APIRequestParams) -> Data? {
        ...
        if let arrParams = remoteRequestParams.arrParams, !arrParams.isEmpty {
            LogHandler.reportLogOnConsole(nil, "arrParams is: \(remoteRequestParams.arrParams ?? [:])")
            for(key, values) in arrParams {
                if let strValues = values as? [String] {
